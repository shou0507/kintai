...
@section('content')
    <div class="art-page">
        <div class="art-title-row">
            ...
            <h1 class="art-title">{{ $user->name }}さんの勤怠一覧</h1>
        </div>

        <div class="att-monthbar">
            {{-- 前月 --}}
            <a href="{{ url('/admin/attendance/staff/' . $user->id) }}?month={{ $prevMonth }}" class="art-monthbtn">
            ...
            {{-- 翌月 --}}
            <a href="{{ url('/admin/attendance/staff/' . $user->id) }}?month={{ $nextMonth }}" class="art-monthbtn">
                    ...
                    @foreach ($days as $day)
                        ...
                        <tr>
                            ...
                            <td class="col-detail">
                                ...
                                @else
                                    <a href="{{ url('/admin/attendance/staff/' . $user->id) }}?month={{ request('month') ?? ($currentMonthParam ?? $prevMonth) }}&date={{ $key }}"
    ...
    <div class="csv-export-wrap">
        <a class="csv-export-button"
            href="{{ route('admin.attendance.staff.csv', ['id' => $user->id, 'month' => request('month') ?? now()->format('Y-m')]) }}">
            CSV出力
        </a>
    </div>
@endsection
